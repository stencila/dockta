/**
 * Script for generating list of packages to include in images from a
 * JSON spec file and package download stats.
 *
 * See `../compile.sh` for example usages.
...
for (let specFile of process.argv.slice(2)) {
  specFile = path.join(__dirname, 'packages', specFile)
