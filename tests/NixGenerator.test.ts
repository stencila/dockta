...
test('generate:packages', async () => {
  let expectedNixfile = fs.readFileSync(
    path.join(fixture('multi-lang-for-nix'), 'default.nix.expected'),
    'utf8'
  )

  const compiler = new DockerCompiler(urlFetcher)
  let environ = await compiler.compile(
    'file://' + fixture('multi-lang-for-nix'),
    false,
    false
  )

  const generator = new NixGenerator(urlFetcher, undefined)
  let nixfile = generator
    .generate(environ, fixture('multi-lang-for-nix'))
    .split('\n')
    .slice(1)
    .join('\n')
