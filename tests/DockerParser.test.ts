...
test('parse:strings', async () => {
  ...
  // Single label
  environ = (await parser.parse(
    'LABEL maintainer="Joe Bloggs <joe@bloggs.com> (https://bloggs.com/joe)"'
  )) as SoftwareEnvironment
  ...
  // Mutiple labels
  environ = (await parser.parse(
    'LABEL maintainer="Joe Bloggs" description="My image"'
  )) as SoftwareEnvironment
  ...
  environ = (await parser.parse(
    'LABEL maintainer="Joe Bloggs" maintainer="Peter Pan"'
  )) as SoftwareEnvironment
  ...
  for (let prefix of ['org.opencontainers.image', 'org.label-schema']) {
    environ = (await parser.parse(
      `LABEL ${prefix}.description="My image"`
    )) as SoftwareEnvironment
    expect(environ.description).toEqual('My image')
  }
...
test('parse:dockerfile-date', async () => {
  ...
  expect(environ.description && environ.description.substring(0, 23)).toEqual(
