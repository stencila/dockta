...
test('compile:dockerfile-date', async () => {
  const compiler = new DockerCompiler(urlFetcher)
  let environ = await compiler.compile(
    'file://' + fixture('dockerfile-date'),
    false
  )

  expect(
    environ && environ.description && environ.description.substring(0, 23)
  ).toEqual('Prints the current date')
  expect(
    environ && environ.authors && (environ.authors[0] as Person).name
  ).toEqual('Nokome Bentley')
})

test('compile:multi-lang', async () => {
  const compiler = new DockerCompiler(urlFetcher)
  let environ = await compiler.compile(
    'file://' + fixture('multi-lang'),
    false,
    false
  )
  ...
  const aptAddMRAN =
    /(apt-add-repository "deb https:\/\/mran.microsoft.com\/snapshot)\/([\d-]+)\/(bin\/linux\/ubuntu focal-cran40\/)"/
  const actual = fs
    .readFileSync(fixture('multi-lang/.Dockerfile'), 'utf8')
    .replace(aptAddMRAN, '$1/YYYY-MM-DD/$3')
  const expected = fs
    .readFileSync(fixture('multi-lang/Dockerfile.expected'), 'utf8')
    .replace(aptAddMRAN, '$1/YYYY-MM-DD/$3')
